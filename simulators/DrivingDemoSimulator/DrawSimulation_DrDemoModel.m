...
MatlabSimfile_Discrete = fopen('../../problems/DrivingDemoModel/OutFiles/MatlabSimfile_Discrete.txt', 'r');
%BestValueFile = fopen('../../problems/DrivingDemoModel/OutFiles/BestValue.txt');
...
SimData_discrete = fscanf(MatlabSimfile_Discrete, '%f', [12 inf]);
q_sim = SimData_discrete(1, :)';
x_sim = SimData_discrete(2, :)';
b_q  = SimData_discrete(3:10, :)';
zq = SimData_discrete(11, :)';
control = SimData_discrete(12, :)';


%%

figureposition = [0 500 2048 300]; figurelinewidth = 1.5; figurefontsize = 24; markersize = 8;
savedir = './figures/';
...
legend('x(t)', 'Location', [.8,.7,.1,.2]);
set(gca,'YLim',[-2 2], 'FontSize', figurefontsize);
...
% draw q(t)
figure('Position', figureposition)
plot(ceil(q_sim/2), 'b', 'LineWidth', figurelinewidth);
xlabel('t', 'FontSize', figurefontsize); ylabel('q_1', 'FontSize', figurefontsize);
%title('q(t)', 'FontSize', figurefontsize);
hold on;
plot(zq-4, 'ko', 'MarkerSize', markersize, 'LineWidth', figurelinewidth); hold off;
legend('Actual q_1(t)', 'Observation z^q(t)', 'Location', [.8,.7,.1,.2]);
set(gca,'YLim',[-3.5 4.5], 'FontSize', figurefontsize, 'YTick',[-3 -2 -1 1 2 3 4], 'YTickLabel', ['DD'; 'TD'; 'OD'; 'AS';'AT'; 'DS'; 'DT' ]);
...
%draw P(q=='Attentive', t);\
figure('Position', figureposition)
plot(sum(b_q(:, 1:4), 2), 'b', 'LineWidth', figurelinewidth);
