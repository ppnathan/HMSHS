...
MatlabSimfile_Discrete = fopen('../../problems/LaneMergingModel/OutFiles/simResultFile2.txt', 'r');
SimData_discrete = fscanf(MatlabSimfile_Discrete, '%f', [19 inf]);

fclose(MatlabSimfile_Discrete);

q_sim = reshape(SimData_discrete(1, :), 2, []);
x_sim = reshape(SimData_discrete(2:5, :), 4, 2, []);
b_q = reshape(SimData_discrete(6:17, :), 12, 2, []);
zq = reshape(SimData_discrete(18, :), 2, []);
control = reshape(SimData_discrete(19, :), 2, []);
...
plot(reshape(x_sim(1, 1, :) - x_sim(3, 1, :), 1, []), 'b', 'LineWidth', figurelinewidth+0.5); hold on;
plot(reshape(x_sim(1, 2, :) - x_sim(3, 2, :), 1, []), 'r', 'LineWidth', figurelinewidth+0.5);
...
legend('first', 'second', 'Location', [.9,.7,.1,.2]);
set(gca, 'FontSize', figurefontsize);
savepath = [savedir 'positions_3cars.png'];
set(gcf,'PaperPositionMode','auto'); print(gcf, '-dpng', savepath);

% draw (x_t(2), x_t(4))
figure('Position', figureposition)
plot(reshape(x_sim(2, 1, :), 1, []), 'b', 'LineWidth', figurelinewidth+0.5); hold on;
plot(reshape(x_sim(2, 2, :), 1, []), 'r', 'LineWidth', figurelinewidth+0.5);
plot(reshape(x_sim(4, 1, :), 1, []), 'k', 'LineWidth', figurelinewidth+0.5);
xlabel('t', 'FontSize', figurefontsize); ylabel('Velocities', 'FontSize', figurefontsize);
%title('v(t)', 'FontSize', figurefontsize);hold on;
legend('first', 'second', 'mine', 'Location', [.9,.7,.1,.2]);
set(gca, 'FontSize', figurefontsize);
savepath = [savedir 'velocities_3cars.png'];
set(gcf,'PaperPositionMode','auto'); print(gcf, '-dpng', savepath);
...
% draw sigma(t)
figure('Position', figureposition)
plot(control(1, :), 'b', 'LineWidth', figurelinewidth);
...
savepath = [savedir 'sigmas_3cars.png'];
set(gcf,'PaperPositionMode','auto'); print(gcf, '-dpng', savepath);
...
for i = 1:size(x_sim, 3)
    ...
    rectangle('Position', [x_sim(1, 1, i) - length_car*0.5, 1 - width_car*0.5, length_car, width_car], ...
        'FaceColor', 'b');
    rectangle('Position', [x_sim(1, 2, i) - length_car*0.5, 1 - width_car*0.5, length_car, width_car], ...
        'FaceColor', 'r');
    
    if(x_sim(3, 1, i) < change_lane_pos)
        rectangle('Position', [x_sim(3, 1, i) - length_car*0.5, -1 - width_car*0.5, length_car, width_car], ...
            'FaceColor', 'k');
    elseif(x_sim(3, 1, i) >= change_lane_pos && x_sim(3, 1, i) < 0)
        y_tmp = - 2 / change_lane_pos * (x_sim(3, 1, i) - change_lane_pos/2);
        rectangle('Position', [x_sim(3, 1, i) - length_car*0.5, y_tmp - width_car*0.5, length_car, width_car], ...
            'FaceColor', 'k');
    else 
        rectangle('Position', [x_sim(3, 1, i) - length_car*0.5, 1 - width_car*0.5, length_car, width_car], ...
            'FaceColor', 'k');
