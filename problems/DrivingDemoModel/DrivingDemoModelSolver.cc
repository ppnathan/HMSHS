...
Number of discrete observations: %d\n Number of discrete controls: %d\n Discount: %f\n ", hmshsmodel.getNumCStateVar(), hmshsmodel.getNumDState(), hmshsmodel.getNumCObsVar(), 
        ...
        NewSolver drivingdemosolver(maxIter, precision);
        printf("\nDeclared Solver object.\n");
        
		int start_s=clock();
        NewAlphaSet Alpha_sol = drivingdemosolver.solve(hmshsmodel, initCState, initDState, nBeliefs, thresholddist);
		int stop_s=clock();
...
//    int nSamples = 5000;
    ...
    AlphaSet Alpha_sol = twodmodelsolver.solve(hmshsmodel, initCState, initDState, nBeliefs, zx_samples, zx_interval);
	curr=time(0);
	cout << "current time is: " << ctime(&curr) <<endl;
    printf("\nFinished!\n");
