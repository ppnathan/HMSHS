...
Belief getbelief(const Model& currModel, const Belief& currbelief, const DControl& sigma_k, const DObs& zq_next, const CState& zx_next) {
    Belief b_next;
    ...
    for(int i = 0; i<currModel.getNumDState(); i++) {
        ...
        b_next.DStateProb.push_back( currModel.getDiscreteObsProb(zq_next, i)*currModel.getCStateTransProb(zx_next, i, currbelief.cstate)*tmp );
...
void BeliefSet::SampleBelief(const Model& currModel, const CState& x_init, const DState& q_init, const vector<double>& initQprob) {
    CState x_sample(x_init);
    DState q_sample(q_init);
    //vector<double> prob_sample(currModel.getNumDState(), 0);
    //prob_sample[q_sample] = 1.0;
...
//        cout<<i<<"  ";
        ...
        if(WriteSampleRecords) {
            ...
            for(int j = 0; j<b_sample.DStateProb.size(); j++) myfile<<b_sample.DStateProb[j]<<"  ";
            myfile<<zq_next<<"  ";
        }
