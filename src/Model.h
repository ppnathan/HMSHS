...
typedef vector<double> CState; // defining continuous state
typedef int DObs;   // defining discrete observation
typedef vector<double> CObs;    // defining continuous observation
typedef int DControl;   // defining discrete control

class Model
{
public:
    Model(int numCStateVar, int numDState, int numCObsVar, int numDObs, int numDControls, double discount):
            numCStateVar(numCStateVar), numDState(numDState),numCObsVar(numCObsVar), numDObs(numDObs), numDControls(numDControls), discount(discount)
            {}
            
    /*	return Tx(xx|qq, x)	*/
    virtual double getCStateTransProb(const CState & x_next, const DState& q_next, const CState& x_k) const = 0 ;

	/*	return Tq(qq|q,sigma)	*/
    virtual double getDStateTransProb(const DState& q_next, const DState& q_k, const DControl& simga_k) const = 0;
    ...
    virtual double sample(const DState& q_k, const CState& x_k, const DControl& sigma_k, DState& q_next, CState& x_next, DObs& obs_out) const = 0;

	virtual CState getNextCStateNoNoise(const DState& q_next, const CState& x_k) const = 0;

	virtual vector<double> get1stDerivative(const DState& q, const CState& x) const = 0;
	
	virtual vector<double> get2ndDerivative(const DState& q, const CState& x) const = 0;
	
	virtual vector<vector<vector<double> > >  getRewardCoeff() const = 0;
	
	virtual vector<vector<double> > getCovariance() const = 0; //covariance[q][coeff];
