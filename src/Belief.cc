...
double Belief::BeliefUpdate(const Model& currModel, const DControl& sigma_k, const DObs& zq_next, const CState& zx_next) {
    vector<double> qProb;
    ...
    for(int qq = 0; qq<currModel.getNumDState(); qq++) {
        ...
        qProb.push_back( currModel.getDiscreteObsProb(zq_next, qq)*currModel.getCStateTransProb(zx_next, qq, this->cstate)*tmp );
    }
